...
const setup = () => {
    const onSubmit = jest.fn().mockResolvedValueOnce({data: {playlistName: 'Test Playlist', description: "testing" }})
...
test('renders playlist form',  () => {
 ...
 test('Submits with Playlist Inputs',  () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith( PlaylistName.value,
        Description.value,
    );
