...
export default (app: any) => {
    app.post('/api/searchalbums', authorizeToken, async (req: Request, res: Response) => {
        try {
            const searchTerm = req.body.search;

            const response = await axios.get(`https://api.spotify.com/v1/search?q=${searchTerm}&type=album`, {
                headers: {
    ...
    app.post('/api/searchalbumtracks', authorizeToken, async (req: Request, res: Response) => {
        try {
